...
func TestServerStopDCGMHealthCache(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	// Create a mock health cache
	...
	healthCache := nvidiadcgm.NewHealthCache(ctx, dcgmInst, time.Minute)

	// Start the cache to spawn the polling goroutine
	if err := healthCache.Start(); err != nil {
		t.Fatalf("Failed to start health cache: %v", err)
	}

	// Create server with health cache
	s := &Server{
		gpudInstance: &components.GPUdInstance{
			DCGMHealthCache: healthCache,
		},
		componentsRegistry: components.NewRegistry(nil),
	}

	// Stop should cleanly shutdown the health cache
	s.Stop()

	// Verify Stop() didn't panic and completed successfully
	// The health cache's polling goroutine should have been stopped via context cancellation
	lastUpdate := healthCache.GetLastUpdateTime()
	t.Logf("DCGM health cache stopped successfully, last update: %v", lastUpdate)
}

func TestServerStopWithNilGPUdInstance(t *testing.T) {
	s := &Server{
		gpudInstance: nil,
	}

	// Should not panic with nil gpudInstance
	s.Stop()
}
