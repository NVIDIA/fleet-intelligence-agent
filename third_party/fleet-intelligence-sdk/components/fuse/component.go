...
import (
	...
	"github.com/NVIDIA/fleet-intelligence-sdk/pkg/fuse"
...
// InjectFault replaces the list connections function with an error-returning version
func (c *component) InjectFault(errMsg string) {
	c.listConnectionsFunc = func() (fuse.ConnectionInfos, error) {
		return nil, fmt.Errorf("injected fuse fault: %s", errMsg)
	}
}

// ClearFault restores the original list connections function
func (c *component) ClearFault() {
	c.listConnectionsFunc = fuse.ListConnections
}

// InjectEvent injects an event into the fuse component's event bucket
func (c *component) InjectEvent(name, eventType, message string) error {
	if c.eventBucket == nil {
		return fmt.Errorf("fuse component has no event bucket")
	}
