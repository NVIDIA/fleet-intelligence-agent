...
import (
	...
	"github.com/NVIDIA/fleet-intelligence-sdk/pkg/pci"
)
...
// InjectFault injects a fault into the PCI component
func (c *component) InjectFault(errMsg string) {
	c.getPCIDevicesFunc = func(ctx context.Context) (pci.Devices, error) {
		return nil, fmt.Errorf("injected PCI fault: %s", errMsg)
	}
}

// ClearFault restores the original PCI devices function
func (c *component) ClearFault() {
	c.getPCIDevicesFunc = pci.List
}

// InjectEvent injects an event into the PCI component's event bucket
func (c *component) InjectEvent(name, eventType, message string) error {
	if c.eventBucket == nil {
		return fmt.Errorf("PCI component has no event bucket")
	}
