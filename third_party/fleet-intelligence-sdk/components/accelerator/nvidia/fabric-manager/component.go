...
import (
	...
	netutil "github.com/NVIDIA/fleet-intelligence-sdk/pkg/netutil"
...
func (c *component) Check() components.CheckResult {
	...
	// If fabric manager state is not supported && IS_RUNTIME_K8S is set, skip all legacy fabric-manager checks.
	isRuntimeK8s, err := strconv.ParseBool(strings.TrimSpace(os.Getenv("IS_RUNTIME_K8S")))
	if err == nil && isRuntimeK8s {
		...
		cr.reason = appendReason(cr.reason, "Agent is running in Kubernetes runtime which does not support fabric manager daemon; skipping fabric manager checks")
		return cr
	}
