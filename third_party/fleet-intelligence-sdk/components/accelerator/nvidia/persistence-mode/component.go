...
func (c *component) Check() components.CheckResult {
	...
	if len(devices) == 0 {
		if c.gpuProvider != nil && c.gpuProvider.GPUDetected() {
			...
			cr.reason = "GPU hardware is detected, but DCGM inventory is unavailable"
			return cr
		}
	...
	if len(disabledGPUs) == supportedCount {
		cr.reason = fmt.Sprintf("persistence mode is disabled on all %d supported GPU(s)", supportedCount)
	} else {
