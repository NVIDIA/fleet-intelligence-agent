...
// InjectFault injects a fault into the memory component by replacing the getMemoryFunc
func (c *component) InjectFault(errMsg string) {
	c.getMemoryFunc = func(uuid string, dev device.Device, productName string, getVirtualMemoryFunc GetVirtualMemoryFunc) (Memory, error) {
		return Memory{}, errors.New(errMsg)
	}
}

// ClearFault clears any injected faults and restores the original memory checking function
func (c *component) ClearFault() {
	c.getMemoryFunc = GetMemory
}
