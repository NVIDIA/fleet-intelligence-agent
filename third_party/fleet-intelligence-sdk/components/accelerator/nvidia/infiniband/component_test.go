...
func TestNew(t *testing.T) {
	...
	assert.Equal(t, defaultHealthCheckInterval, casted.checkInterval)
}

func TestNewWithHealthCheckIntervalOverride(t *testing.T) {
	t.Parallel()

	ctx := context.Background()
	override := 12 * time.Second
	gpudInstance := &components.GPUdInstance{
		RootCtx:              ctx,
		NVIDIAToolOverwrites: pkgconfigcommon.ToolOverwrites{},
		HealthCheckInterval:  override,
	}
	...
	casted := comp.(*component)
	assert.Equal(t, override, casted.checkInterval)
}
