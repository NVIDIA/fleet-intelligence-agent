// Package utilization tracks NVIDIA GPU utilization metrics via DCGM.
package utilization
...
const Name = "accelerator-nvidia-dcgm-utilization"
...
func (c *component) Check() components.CheckResult {
	log.Logger.Infow("checking nvidia gpu utilization via DCGM")
	...
	// Query and export DCGM utilization field values for all devices
	deviceValues, err := c.dcgmFieldValueCache.GetResult(utilizationFields)
	if err != nil {
		if nvidiadcgm.IsUnhealthyAPIError(err) {
			cr.health = apiv1.HealthStateTypeUnhealthy
			cr.reason = nvidiadcgm.AppendDCGMErrorType("failed to get DCGM utilization fields", err)
		} else {
			// Unknown error - treat as degraded
			cr.health = apiv1.HealthStateTypeDegraded
			cr.reason = fmt.Sprintf("failed to get DCGM utilization fields: %v", err)
		}
		cr.err = err
		return cr
	} else {
		for _, deviceData := range deviceValues {
			...
			case dcgm.DCGM_FI_DEV_GPU_UTIL:
				metricDCGMFIDevGPUUtil.With(prometheus.Labels{
					"uuid":      deviceData.UUID,
					"gpu": fmt.Sprintf("%d", deviceData.DeviceID),
				}).Set(float64(fieldValue.Int64()))
			case dcgm.DCGM_FI_DEV_MEM_COPY_UTIL:
				metricDCGMFIDevMemCopyUtil.With(prometheus.Labels{
					"uuid":      deviceData.UUID,
					"gpu": fmt.Sprintf("%d", deviceData.DeviceID),
				}).Set(float64(fieldValue.Int64()))
			case dcgm.DCGM_FI_DEV_ENC_UTIL:
				metricDCGMFIDevEncUtil.With(prometheus.Labels{
					"uuid":      deviceData.UUID,
					"gpu": fmt.Sprintf("%d", deviceData.DeviceID),
				}).Set(float64(fieldValue.Int64()))
			case dcgm.DCGM_FI_DEV_DEC_UTIL:
				metricDCGMFIDevDecUtil.With(prometheus.Labels{
					"uuid":      deviceData.UUID,
					"gpu": fmt.Sprintf("%d", deviceData.DeviceID),
				}).Set(float64(fieldValue.Int64()))
				}
	...
	// Utilization metrics are informational only - always return healthy
	cr.health = apiv1.HealthStateTypeHealthy
	cr.reason = "utilization metrics collected successfully"
