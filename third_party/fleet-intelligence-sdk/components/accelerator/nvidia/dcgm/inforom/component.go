...
// Package inforom tracks NVIDIA GPU InfoROM metrics via DCGM.
package inforom
...
func (c *component) Check() components.CheckResult {
	...
	// Get cached DCGM InfoROM health check result from shared cache
	healthResult, incidents, err := c.dcgmHealthCache.GetResult(dcgm.DCGM_HEALTH_WATCH_INFOROM)
	if err != nil {
		if nvidiadcgm.IsUnhealthyAPIError(err) {
			cr.health = apiv1.HealthStateTypeUnhealthy
			cr.reason = nvidiadcgm.AppendDCGMErrorType("failed to get DCGM InfoROM health check result", err)
		} else {
			// Unknown error - treat as degraded
			cr.health = apiv1.HealthStateTypeDegraded
			cr.reason = fmt.Sprintf("failed to get DCGM InfoROM health check result: %v", err)
		}
		cr.err = err
		return cr
	}
	...
	case dcgm.DCGM_HEALTH_RESULT_PASS:
		cr.health = apiv1.HealthStateTypeHealthy
		cr.reason = "all InfoROM health checks passed"
