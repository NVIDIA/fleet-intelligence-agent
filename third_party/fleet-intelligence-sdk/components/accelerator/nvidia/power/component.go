...
func (c *component) InjectFault(errMsg string) {
	c.getPowerFunc = func(uuid string, dev device.Device) (Power, error) {
		return Power{}, errors.New(errMsg)
	}
}

// ClearFault clears any injected faults and restores the original power checking function
func (c *component) ClearFault() {
	c.getPowerFunc = GetPower
}
