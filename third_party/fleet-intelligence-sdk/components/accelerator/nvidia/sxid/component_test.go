...
// createSXidEventForComponent creates a sxid event in the format expected by the component
func createSXidEventForComponent(timestamp time.Time, sxid uint64, deviceUUID string) eventstore.Event {
	return eventstore.Event{
		Time: timestamp,
		Name: EventNameErrorSXid,
		ExtraInfo: map[string]string{
			EventKeyErrorSXidData: strconv.FormatUint(sxid, 10),
...
func TestSXIDComponent_Events(t *testing.T) {
	...
	// Create a real sxid event (sxid 11001 is a fatal NVSwitch error - "ingress invalid command")
	testEvents := eventstore.Events{
		createSXidEventForComponent(time.Now(), 11001, "PCI:0000:05:00.0"),
	}
	...
	if len(events) > 0 {
		// Check that at least one of the events matches what we expect (sxid event)
		found := false
		for _, event := range events {
			if event.Name == EventNameErrorSXid {
				found = true
				assert.Equal(t, string(apiv1.EventTypeFatal), string(event.Type))
				assert.Contains(t, event.Message, " [Potential Fatal]")
				assert.Contains(t, event.Message, "11001")                   // Should contain the sxid number
				assert.Contains(t, event.Message, "ingress invalid command") // Should contain the sxid name
				t.Logf("Found sxid event with message: %s", event.Message)
				break
			}
		}
		assert.True(t, found, "Couldn't find the sxid event in the retrieved events")
	}
}
