...
const (
	...
	// DefaultMetadataLookback is the strict window for extra_info XID entries sent to the backend.
	...
	for {
		...
		case message := <-kmsgCh:
			...
			if xidErr.Detail != nil {
				...
				xidPayload.ErrorStatusHex = fmt.Sprintf("0x%08x", xidPayload.ErrorStatus)
			}
			if parsed := ExtractNVRMXidInfoExtended(message.Message); parsed != nil {
				xidPayload.IntrinfoHex = fmt.Sprintf("0x%08x", parsed.Intrinfo)
				xidPayload.ErrorStatus = parsed.ErrorStatus
				xidPayload.ErrorStatusHex = fmt.Sprintf("0x%08x", parsed.ErrorStatus)
			}
...
func (c *component) updateCurrentState() error {
	...
	c.mu.Lock()
	c.currState = evolveHealthyState(events, c.devices, rebootThreshold.Threshold, now)
	if rebootErr != "" {
