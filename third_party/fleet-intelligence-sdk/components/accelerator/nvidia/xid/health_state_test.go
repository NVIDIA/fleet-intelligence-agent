...
func Test_xidErrorEventDetailJSON(t *testing.T) {
	...
	t.Run("successful marshaling", func(t *testing.T) {
		xidErr := xidErrorEventDetail{
			...
			RawKmsg:        "NVRM: Xid (PCI:0000:04:00): 149, NETIR Fatal ...",
			Xid:            123,
			IntrinfoHex:    "0x004505c6",
			ErrorStatus:    0x00000000,
			ErrorStatusHex: "0x00000000",
			SuggestedActionsByGPUd: &apiv1.SuggestedActions{
		...
		assert.Equal(t, xidErr.RawKmsg, unmarshaled.RawKmsg)
		assert.Equal(t, xidErr.Xid, unmarshaled.Xid)
		assert.Equal(t, xidErr.IntrinfoHex, unmarshaled.IntrinfoHex)
		assert.Equal(t, xidErr.ErrorStatus, unmarshaled.ErrorStatus)
		assert.Equal(t, xidErr.ErrorStatusHex, unmarshaled.ErrorStatusHex)
