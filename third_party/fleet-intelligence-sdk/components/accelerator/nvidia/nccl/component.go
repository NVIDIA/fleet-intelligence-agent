...
func (c *component) InjectFault(errMsg string) {
	c.readAllKmsg = func(ctx context.Context) ([]kmsg.Message, error) {
		return nil, fmt.Errorf("injected NCCL fault: %s", errMsg)
	}
}

// ClearFault restores the original kmsg reading function
func (c *component) ClearFault() {
	c.readAllKmsg = kmsg.ReadAll
}

// InjectEvent injects an event into the NCCL component's event bucket
func (c *component) InjectEvent(name, eventType, message string) error {
	if c.eventBucket == nil {
		return fmt.Errorf("NCCL component has no event bucket")
	}
