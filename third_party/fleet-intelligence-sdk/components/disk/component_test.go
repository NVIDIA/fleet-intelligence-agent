...
func TestDataGetStates(t *testing.T) {
	cr := &checkResult{
		LocalPartitions: disk.Partitions{
		...
		reason: "found 1 local partitions and 1 block devices",
	}

	states := cr.HealthStates()
	assert.Len(t, states, 1)
	assert.Equal(t, "disk", states[0].Name)
	assert.Equal(t, "found 1 local partitions and 1 block devices", states[0].Reason)
	assert.Equal(t, apiv1.HealthStateTypeHealthy, states[0].Health)
...
func TestCheckOnce(t *testing.T) {
	...
	t.Run("successful check", func(t *testing.T) {
		...
		assert.Len(t, lastCheckResult.LocalPartitions, 1)
	})
	...
	t.Run("no local partitions", func(t *testing.T) {
		c := createTestComponent(ctx, []string{}, []string{})
		defer func() {
			_ = c.Close()
		}()
...
func TestMetricsTracking(t *testing.T) {
	...
	assert.Equal(t, apiv1.HealthStateTypeHealthy, lastCheckResult.health)
	assert.Len(t, lastCheckResult.LocalPartitions, 1)
	assert.Equal(t, mockPartition.MountPoint, lastCheckResult.LocalPartitions[0].MountPoint)
}
...
func TestCheckResultString(t *testing.T) {
	...
	t.Run("empty LocalPartitions", func(t *testing.T) {
		cr := &checkResult{
			LocalPartitions: disk.Partitions{},
		}
		result := cr.String()
	...
	t.Run("LocalPartitions with nil Usage", func(t *testing.T) {
		cr := &checkResult{
			LocalPartitions: disk.Partitions{
				{
					Device:     "/dev/sda1",
	...
	t.Run("LocalPartitions with valid Usage", func(t *testing.T) {
		cr := &checkResult{
			LocalPartitions: disk.Partitions{
				{
					Device:     "/dev/sda1",
...
func TestCombinedPartitions(t *testing.T) {
	...
	assert.Len(t, lastCheckResult.LocalPartitions, 1)
	assert.Len(t, lastCheckResult.NFSPartitions, 1)
	assert.Equal(t, mockExt4Partition.MountPoint, lastCheckResult.LocalPartitions[0].MountPoint)
	assert.Equal(t, mockNFSPartition.MountPoint, lastCheckResult.NFSPartitions[0].MountPoint)
}
...
func TestNFSPartitionsStringMethod(t *testing.T) {
	// The String() method checks for len(LocalPartitions) == 0 and returns an empty string if true
...
func TestNewComponentMoreCases(t *testing.T) {
	...
	assert.NotNil(t, component.getLocalPartitionsFunc)
...
// TestComponent_StatTimedOut_LocalPartitionsIgnored tests that only NFS partitions are checked for StatTimedOut
func TestComponent_StatTimedOut_LocalPartitionsIgnored(t *testing.T) {
	ctx := context.Background()
	mockDevice := disk.BlockDevice{
		Name: "sda",
		Type: "disk",
	...
	assert.Len(t, lastCheckResult.LocalPartitions, 1)
	assert.Len(t, lastCheckResult.NFSPartitions, 1)
	assert.True(t, lastCheckResult.LocalPartitions[0].StatTimedOut) // Ext4 partition has StatTimedOut=true
...
func TestComponent_StatTimedOut_NoNFSPartitions(t *testing.T) {
	...
	assert.True(t, lastCheckResult.LocalPartitions[0].StatTimedOut)
}
...
func TestComponent_LocalPartitionsTimeoutSetsReasonAndError(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	c.getNFSPartitionsFunc = func(ctx context.Context) (disk.Partitions, error) {
		t.Fatalf("getNFSPartitionsFunc should not be called when local partitions fail")
		return nil, nil
	}
	...
	assert.Equal(t, "failed to get local partitions -- took too long", cr.reason)
...
func TestComponentWithMountPointFiltering(t *testing.T) {
	...
	t.Run("check filters empty and provider-specific mount points", func(t *testing.T) {
		...
		assert.NotNil(t, lastCheckResult)
		assert.Equal(t, apiv1.HealthStateTypeHealthy, lastCheckResult.health)
		assert.Len(t, lastCheckResult.LocalPartitions, 2)
		assert.Len(t, lastCheckResult.NFSPartitions, 1)

		// Verify none have empty mount points (as they would be filtered by the disk package)
		for _, p := range lastCheckResult.LocalPartitions {
...
func TestDiskUsageThresholds(t *testing.T) {
	...
	t.Run("ext4 root partition exceeds degraded threshold", func(t *testing.T) {
		...
		assert.Contains(t, cr.reason, "root partition /: free space 15 GiB is below 20 GiB threshold (100 GiB total)")
	})
	...
	t.Run("default thresholds on rootfs", func(t *testing.T) {
		...
		assert.Equal(t, apiv1.HealthStateTypeDegraded, cr.health)
		assert.Contains(t, cr.reason, "root partition /: free space 400 MiB is below 500 MiB threshold")
	})
	...
	t.Run("combined with other failure reasons (rootfs + NFS)", func(t *testing.T) {
		...
		assert.Contains(t, cr.reason, "root partition /: free space 4.0 GiB is below 10 GiB threshold")
