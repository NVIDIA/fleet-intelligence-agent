...
import (
	...
	"github.com/NVIDIA/fleet-intelligence-sdk/pkg/pstore"
)
...
// InjectFault injects a fault into the OS component
func (c *component) InjectFault(errMsg string) {
	c.countProcessesByStatusFunc = func(ctx context.Context) (map[string][]process.ProcessStatus, error) {
		return nil, fmt.Errorf("injected OS fault: %s", errMsg)
	}
}

// ClearFault restores the original OS component
func (c *component) ClearFault() {
	c.countProcessesByStatusFunc = process.CountProcessesByStatus
}

// InjectEvent injects an event into the OS component's event bucket
func (c *component) InjectEvent(name, eventType, message string) error {
	if c.eventBucket == nil {
		return fmt.Errorf("OS component has no event bucket")
	}
