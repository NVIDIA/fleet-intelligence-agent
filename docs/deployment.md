...
- `DCGM_ADDRESS=nvidia-dcgm.gpu-operator.svc:5555`
...
`gpuhealth-agent` to talk to via `DCGM_ADDRESS`.
...
- `gpuhealth-agent` consumes DCGM: `gpuhealth-agent` talks to DCGM over the
  cluster network using `DCGM_ADDRESS`. If DCGM is missing or unreachable,
