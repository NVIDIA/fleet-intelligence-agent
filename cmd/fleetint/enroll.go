...
func enrollCommand(cliContext *cli.Context) error {
	...
	customerID := cliContext.String("customer-id")
	sakToken := cliContext.String("token")

	// Validate: exactly one mode must be specified.
	if gatewayEndpoint == "" && (baseEndpoint == "" || customerID == "" || sakToken == "") {
		return fmt.Errorf("either --gateway or --endpoint + --customer-id + --token are required")
	}
	if gatewayEndpoint != "" && (baseEndpoint != "" || customerID != "" || sakToken != "") {
		return fmt.Errorf("--gateway and --endpoint/--customer-id/--token are mutually exclusive")
	}
	...
	return enrollDirect(dbRW, baseEndpoint, sakToken, customerID)
}
...
func enrollDirect(dbRW *sql.DB, baseEndpoint, sakToken, customerID string) error {
	...
	jwtToken, err := enrollment.PerformEnrollment(context.Background(), enrollEndpoint, sakToken, customerID)
	if err != nil {
		return err
	}
	...
	return storeConfigInMetadata(dbRW, enrollEndpoint, metricsEndpoint, logsEndpoint, nonceEndpoint, jwtToken, sakToken, customerID)
}

func storeConfigInMetadata(dbRW *sql.DB, enrollEndpoint, metricsEndpoint, logsEndpoint, nonceEndpoint, jwtToken, sakToken, customerID string) error {
	ctx := context.Background()

	if err := pkgmetadata.SetMetadata(ctx, dbRW, "sak_token", sakToken); err != nil {
		return fmt.Errorf("failed to set SAK token: %w", err)
	}
	if err := pkgmetadata.SetMetadata(ctx, dbRW, "customer_id", customerID); err != nil {
		return fmt.Errorf("failed to set customer ID: %w", err)
	}
	if err := pkgmetadata.SetMetadata(ctx, dbRW, pkgmetadata.MetadataKeyToken, jwtToken); err != nil {
		return fmt.Errorf("failed to set JWT token: %w", err)
	}
