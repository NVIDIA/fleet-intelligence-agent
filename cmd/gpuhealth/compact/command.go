...
func Command(cliContext *cli.Context) error {
	...
	portOpen := netutil.IsPortOpen(gpuhealthconfig.DefaultHealthPort) // gpuhealth uses port 15133
	if portOpen {
		return fmt.Errorf("gpuhealth is running on port %d (must be stopped before running compact)", gpuhealthconfig.DefaultHealthPort)
	}
